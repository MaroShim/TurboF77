...
func (u *UserScreen) Draw(screen tcell.Screen, width, height int) {
	...
	// Bottom status banner: [ Turbo F77 User Screen (Alt+F5) - Press any key to return ]
	banner := " [ Turbo F77 User Screen (Alt+F5) - Press any key to return to IDE ] "
	bx := (width - len(banner)) / 2
	if bx < 0 {
		bx = 0
	}
	for c := 0; c < width; c++ {
		screen.SetContent(c, height-1, ' ', nil, barStyle)
	}
