...
// SetColumnGuidesEnabled updates the Column Guides item label in Options menu
func (m *MenuBar) SetColumnGuidesEnabled(enabled bool) {
	for i := range m.Menus {
		if m.Menus[i].Title == "Options" {
			for j := range m.Menus[i].Items {
				if m.Menus[i].Items[j].ActionID == "options_toggle_guides" {
					if enabled {
						m.Menus[i].Items[j].Label = "Column Guides: ON"
					} else {
...
// TriggerItemByHotKey checks if any item in the currently active menu has a label starting with r
func (m *MenuBar) TriggerItemByHotKey(r rune) string {
	if !m.OpenDropdown || m.ActiveMenu < 0 || m.ActiveMenu >= len(m.Menus) {
		return ""
	}
	target := unicode.ToUpper(r)
	for _, item := range m.Menus[m.ActiveMenu].Items {
		if item.IsSep || item.Disabled || item.ActionID == "" {
			continue
		}
		runes := []rune(item.Label)
		if len(runes) > 0 && unicode.ToUpper(runes[0]) == target {
			return item.ActionID
		}
	}
	return ""
