...
func (o *OpenFileDialog) RefreshFiles() {
	...
	for _, entry := range entries {
		...
		} else if compiler.IsFortranSource(name) || strings.EqualFold(name, "makefile") || strings.HasSuffix(name, ".txt") {
			o.Files = append(o.Files, name)
		}
	}
...
func (o *OpenFileDialog) Draw(screen tcell.Screen, screenW, screenH int) {
	...
	ui.DrawButton(screen, x+30, y+dialogH-3, 12, "Cancel", false)
}
...
func (o *OpenFileDialog) HandleMouse(mx, my int, btn tcell.ButtonMask, screenW, screenH int) bool {
	...
	if btn&tcell.Button1 != 0 {
		...
		// Cancel button: x+30, y+dialogH-3, width 12
		if my == y+dialogH-3 && mx >= x+30 && mx < x+30+12 {
			o.Hide()
			return true
		}
