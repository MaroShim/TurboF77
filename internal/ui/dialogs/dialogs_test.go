...
func TestConfirmSaveDialog(t *testing.T) {
	...
	d.Show("test.for", func(choice ConfirmChoice) {
		selectedChoice = choice
	})

	if !d.IsVisible() || d.FileName != "test.for" || d.SelectedIndex != 0 {
		t.Fatalf("unexpected state after Show: %+v", d)
	}
	...
	// Test Choose Yes
	d.Show("main.for", func(choice ConfirmChoice) {
		selectedChoice = choice
	})
	d.Choose(ConfirmYes)
...
func TestDialogs_HandleMouse(t *testing.T) {
	...
	// Click Cancel button: x+30, y+dialogH-3
	openDlg.HandleMouse(x+32, y+dialogH-3, tcell.Button1, screenW, screenH)
	if openDlg.IsVisible() {
	...
	confirmDlg.Show("test.for", func(choice ConfirmChoice) {
