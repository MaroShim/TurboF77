...
func TestEditor_MouseClickAndSelection(t *testing.T) {
	...
	// 1. Click on line 1, column 6
	handled := ed.HandleMouseClick(interiorX, interiorY, interiorW, interiorH, interiorX+6, interiorY+1, false)
	if !handled {
	...
	if ed.CursorX != 6 {
		t.Errorf("expected CursorX 6, got %d", ed.CursorX)
	}

	// 2. Drag
	handled = ed.HandleMouseClick(interiorX, interiorY, interiorW, interiorH, interiorX+11, interiorY+1, true)
	if !handled {
	...
	if selected != "PRINT" {
		t.Errorf("expected selected text 'PRINT', got %q", selected)
	}

	// 3. ScrollLines
	ed.ScrollLines(5)
	if ed.ScrollY != 2 {
		t.Errorf("expected ScrollY capped at 2, got %d", ed.ScrollY)
	}
	ed.ScrollLines(-10)
	if ed.ScrollY != 0 {
